...
if(redraw==false){
        ...
        Tile<Integer> tile = graph.getGraph()[i][j];
        sources_a[i][j].setLocation(Xpos+drawBorder /8-drawBorder /20,Ypos+1);
        sources_a[i][j].setSize(drawBorder /20,drawBorder /20);
        sinks_a[i][j].setLocation(Xpos+1,Ypos+drawBorder / 8-drawBorder /20);
        sinks_a[i][j].setSize(drawBorder /20,drawBorder /20);
        add(sources_a[i][j]);
        add(sinks_a[i][j]);
        int gap_x = 0;
        int gap_y = 0;
